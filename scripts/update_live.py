...
# Some picks were entered with a short/incorrect ticker that doesn't match
# the real NSE symbol Yahoo Finance uses, so the lookup silently fails.
# Add an entry here whenever a stock shows up as SKIP/FAIL in the Action
# log for that reason.
SYMBOL_FIXES = {
    "JAYNECO": "JAYNECOIND",  # Jayaswal Neco Industries Ltd
}

# Safety guard: Yahoo Finance occasionally returns a bad/stale/glitched
# quote for a symbol on a given run (this happened for INDIGRID — a real
# ~175 stock briefly came back as ~140, a fake -20% "move"). Any single-run
# move bigger than this percent vs. the price already stored in data.json
# is treated as suspicious and SKIPPED (not written into live.json) instead
# of shown to visitors as if it were real.
MAX_SANE_MOVE_PCT = 15
...
def collect_symbols(data):
    """Ticker + name of every currently-open pick, plus a raw-string ->
    stored currentPrice map (used as the sanity-check baseline below)."""
    raw_symbols = []
    baseline = {}
...
def main():
    ...
    raw_symbols, baseline = collect_symbols(data)
    ...
    for sym, raw in seen.items():
        ...
        try:
            ...
            if price is None:
                ...
                continue
            old = baseline.get(raw)
            if old and old > 0:
                move_pct = abs(price - old) / old * 100
                if move_pct > MAX_SANE_MOVE_PCT:
                    print("SUSPICIOUS %s (%s): stored %s vs fetched %s looks like a %.1f%% move — "
                          "skipping, not showing this to visitors as live." % (sym, raw, old, price, move_pct))
                    continue

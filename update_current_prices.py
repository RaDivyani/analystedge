...
# ~175 stock briefly came back as ~140, a fake -20% "move" that then got
# written permanently into data.json and stayed wrong until someone
# manually fixed it). Rather than trust every fetch blindly, any single-run
# move bigger than this percent is treated as suspicious and SKIPPED
# (old price kept, logged clearly) instead of applied automatically.
...
def main():
    ...
    for sym, plist in by_symbol.items():
        ...
        price = round(price, 2)
        for p in plist:
            old = safe_num(p.get("currentPrice"))
            if old and old > 0:
                move_pct = abs(price - old) / old * 100
                if move_pct > MAX_SANE_MOVE_PCT:
                    print("SUSPICIOUS %s (%s): %s -> %s looks like a %.1f%% move in one run — "
                          "skipping, please verify manually instead of trusting this blindly."
                          % (sym, p.get("name"), old, price, move_pct))
                    continue
